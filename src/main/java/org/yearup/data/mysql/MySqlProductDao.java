...
public class MySqlProductDao extends MySqlDaoBase implements ProductDao
{
    ...
    public List<Product> search(Integer categoryId, BigDecimal minPrice, BigDecimal maxPrice, String subCategory)
    {
        ...
        String sql = "SELECT * FROM products " +
                ...
                "   AND (price >= ? OR ? = -1) " +
                "   AND (price <= ? OR ? = -1) " +      // added this line to ensure the min to max pricing is correct logic
                "   AND (subcategory = ? OR ? = '') ";
        ...
        try (Connection connection = getConnection())
        {
            ...
            statement.setBigDecimal(4, minPrice); // added an extra min price to match the logic behind the sql query above
            statement.setBigDecimal(5, maxPrice); // same as above ^
